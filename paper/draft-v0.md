...
轉包給 C,可見性就沒了。§6.2 那支至今仍紅的 probe 就是這句話的另一面。
...
4. 一項權利可以搭著那個結構走而中介從未定義它,由持有者在 runtime 行使,
   到邊界為止——兩個實例,其中一個沒有實驗性但書(§5)。
5. **兩個我們對著跑得起來的系統證偽掉的性質**(§6):來源不等於持有;完整性
...
> **機制章,而且是 EMAS 審稿人讀最兇的一節。** §4.1–4.4 是機制,§4.5 是
> 中間人的必要性論證——那一節不能省,因為簽名鏈本身是離線可驗的,審稿人
> 一定會問「那要你幹嘛」。
...
> §4.5 說明為什麼這正是需要中間人的理由,§6.2 說明為什麼即使有了中間人,
> 這件事仍然只被收窄而沒有被關閉。
...
§6.2 那支至今仍紅的 probe 正是這句話的另一面:本文並不主張已經關閉 (ii),
...
- **持有。** 一條簽好的鏈證明**來源**,不證明**現在呈交它的人**握有那把
  ...
  認證呈交者需要一個握著連線的參與方,而門收到的是 bytes(§6.1)。
- **暫停。** 一個 run 活得比任何一條連線久。誰有權回答一個暫停,取決於
  「這一段的 head 是誰」——而那件事只有持有這個 run 的參與方知道(§5)。
