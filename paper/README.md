...
| 檔案 | 放什麼 | 不放什麼 |
|---|---|---|
| **`bibliography-notes.md`** | 條目索引：每一條引用附「它支持哪個主張」，按論文的規劃章節排 | 論證、判斷、進度；**逐字引文**（在 `reference/`） |
| **`reference/`** | **收據**：一個來源一個檔，附作者、日期、**讀到哪**、以及**草稿逐字引用的那些句子**，每句標明出現在哪一節、承擔什麼主張 | 「它支持哪個主張」的地圖（在 `bibliography-notes.md`） |
| **`landscape-2026-08-25.md`** | 產品與規格側的重查（兩輪）：五份委派鏈草案、四家大廠自己寫下的邊界、對論文的影響清單 | 學術文獻導讀 |
...
- **想引用什麼** → `bibliography-notes.md`（找得到哪一篇支持什麼）
- **要把某一句引文寫進正文** → `reference/`（那一句的逐字原文、讀到哪、
  以及它會怎麼害你）。**五個來源目前還不能引**，包括 §1 開場的那一個
