...
| [`funduq-llm-provider-sdk/`](funduq-llm-provider-sdk/) | **What an LLM provider and funduq agree on** — the party answering KYOK completions: the port, the delivered-completion envelope, the structured refusal, its own registration payloads. Depends on `funduq-provider-sdk` (identity is shared) and `openai` (types only), not `funduq`. See its [README](funduq-llm-provider-sdk/README.md) |
