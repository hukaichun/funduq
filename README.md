...
- **Keep Your Own Key** *(experimental)*: a relay that lets callers fund LLM inference with their own API keys without handing the raw credential to agent hosts. See [Keep your own key](https://github.com/hukaichun/funduq/blob/d78d0638c0ec2126167240c62471651b5468d35b/design/keep-your-own-key.md), a working note kept in history rather than the tree.
...
The same principles run through conversation semantics, implemented and tested: who may speak on a bound thread, and who may answer or stop its paused runs, is decided by signature — responsibility chains, specified for callers in [the integration contract](docs/integration-contract.md) and enforced in code (`doors.open_run` → `identity.verify_resolution`; `funduq/tests/test_responsibility_chains.py`). Speaking to work already in flight is a declared, opt-in extension (interjection, same page). Parts of it are under open discussion with the protocol communities: [who answers a delegated `input-required`](https://github.com/a2aproject/A2A/discussions/2148), [the multi-turn gap list](https://github.com/a2aproject/A2A/issues/1992), and [in-flight steering for AG-UI](https://github.com/ag-ui-protocol/ag-ui/issues/2148).
...
| **Decides who may speak?** | Only what a caller's own signatures prove: a thread bound to a [responsibility chain](docs/integration-contract.md) admits its head and the serving provider and refuses the rest. Beyond that, nothing tells core who is on the other end | Yes. Authentication, authorization, rate limits, and whatever gate a deployment wants in front of the door |
...
Schema changes are Alembic revisions packaged under [`funduq/funduq/alembic/`](funduq/funduq/alembic/) — `uv run alembic upgrade head`, run from `funduq/`, is a deploy-time DDL step, deliberately separate from anything a running gateway does (see `CONTRIBUTING.md`). Deployments that will not let an application migrate its own database have three supported tiers.
...
| [`docs/`](docs/) | The published site — [what each half is for](docs/responsibilities.md), the [integration contract](docs/integration-contract.md), the agent loop, what a deployment has to know, the SDK pages, the [contract changelog](docs/contract-changelog.md), and [`contract-vectors.json`](docs/contract-vectors.json), the byte-level authority behind them. The *why* lives in git history, not the tree |
...
- **Public key revocation**: a `revoked_keys` blocklist checked **at registration**, so a leaked provider Ed25519 key can be shut out immediately instead of waiting for its holder to stop using it. Written to only by whoever already has direct DB access — no new admin/auth surface inside funduq itself, consistent with funduq having no account system at all. Deliberately **not** consulted by chain verification: a verifier that refused a hop because of a list would be funduq deciding whose keys count, and enforcement against a chain belongs to its consumer, whose policy knows the expected call graph — which is also the only party that can hold a list of its own.
