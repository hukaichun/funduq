...
- **Durable threads, runs, and human-in-the-loop.** Conversation state persists; a run that finishes asking leaves its thread waiting, and a signed answer is the next run. SQLite by default with zero configuration; one `database_url` moves the same code path to Postgres.
...
| [`docs/`](docs/) | The published site — [what each half is for](docs/responsibilities.md), [the integration contract](docs/integration-contract.md), [the provider link](docs/provider-link.md), [operational limits](docs/operational-limits.md), [what a deployment owes](docs/host-obligations.md), the SDK pages, the [contract changelog](docs/contract-changelog.md) and [`contract-vectors.json`](docs/contract-vectors.json). The *why* lives in git history, not the tree |
